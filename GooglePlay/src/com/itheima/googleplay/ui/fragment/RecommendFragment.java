...
import com.itheima.googleplay.http.protocol.RecommendProtocol;
	...
	class RecommendAdapter implements StellarMap.Adapter {
		...
		public int getCount(int group) {
			...
			if (group == getGroupCount() - 1) {
				// 最后一页, 将除不尽,余下来的数量追加在最后一页, 保证数据完整不丢失（不丢下任何一个兄弟）
		...
		public View getView(int group, int position, View convertView) {
			...
			// 随机颜色  r g b, 0-255 -> 30-230, 颜色值不能太小或太大, 从而避免整体颜色过亮或者过暗
		...
		public int getNextGroupOnZoom(int group, boolean isZoomIn) {
			...
			if (isZoomIn) {
				...
				if (group > 0) {
					...
					// 跳到最后一页
					group = getGroupCount() - 1;//第一页跳到最后一页做成循环
				}
			} else {
				// 往上滑加载下一页
				if (group < getGroupCount() - 1) {
